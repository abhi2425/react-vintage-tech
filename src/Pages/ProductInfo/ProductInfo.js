...
const ProductInfo = () => {
   ...
   const { setCartItems, setTotalCount, setTotalPrice, cartItems } = useCartContext()
   const { productId } = useParams()
   const product = products.find((item) => item.id === +productId)

   const addProductInCart = () => {
      const foundDuplicateItemIndex = cartItems.findIndex((prevItem) => prevItem.id === product.id)
         ...
         <main className='productInfo'>
            <section className='productInfoSection'>
               <article className='imageSection'>
                  <img src={product.image.url} alt={product.title} className='image--style' />
               </article>
